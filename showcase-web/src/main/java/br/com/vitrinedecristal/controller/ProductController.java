...
public class ProductController extends SpringBeanAutowiringSupport {
	...
	public void uploadImage(@ApiParam @PathParam("id") Long id, @FormDataParam("file") InputStream imageInputStream) throws BusinessException {
		// try {
		/*
		 * ByteArrayOutputStream out = new ByteArrayOutputStream(); int read = 0; byte[] bytes = new byte[1024]; while ((read = uploadedInputStream.read(bytes)) != -1) { out.write(bytes, 0, read); }
		 */
		// byte[] bytes = IOUtils.toByteArray(uploadedInputStream);
		this.imageService.createImage(id, imageInputStream);
